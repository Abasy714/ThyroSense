...
import { mockPatients, getClassCounts, getAllPredictions, getHighRiskPatients, getAvgConfidence, getPredictionsThisMonth } from '../data/mockPatients'
...
function buildHormoneData() {
  const grouped = {}
  mockPatients.forEach(p => {
    p.predictions.forEach(pred => {
      const cls = pred.predicted_class
      if (!grouped[cls]) grouped[cls] = []
      if (pred.form_data?.TSH != null) grouped[cls].push(pred.form_data.TSH)
    })
  })
  return Object.entries(grouped).map(([name, values]) => ({
    name,
    avgTSH: +(values.reduce((a, b) => a + b, 0) / values.length).toFixed(2),
  }))
}
...
export default function DoctorHome() {
  ...
  const hormoneData = buildHormoneData()
  const avgConf = Math.round(getAvgConfidence() * 100)
  const thisMonth = getPredictionsThisMonth()
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.35 }}>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        <StatCard label="Total Patients" value={mockPatients.length} sub="across all diagnoses" delay={0.05} accent />
